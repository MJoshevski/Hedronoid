    ...
    public class ThirdPersonCamera : PivotBasedCameraRig
    {
        ...
        private void HandleRotationMovement()
        {
            ...
            else
            {
                ...
                if (GravityService.Instance.GravityUp.x == 0) m_TiltAngle -= y * m_TurnSpeed;
                else m_TiltAngle -= x * m_TurnSpeed;

                // and make sure the new value is within the tilt range
                if (GravityService.Instance.GravityUp.x == 0) m_TiltAngle = Mathf.Clamp(m_TiltAngle, -m_TiltMin, m_TiltMax);
