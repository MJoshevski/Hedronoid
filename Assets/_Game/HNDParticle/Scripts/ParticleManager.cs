...
namespace Hedronoid.Particle
{
    public class PlayParticleSystem : HNDBaseEvent
    {
        ...
        public HNDParticleSystem ParticleSystemInstance;
    }
    ...
    public class ParticleManager : HNDPoolManager
    {
        public class PlayConfig
        {
            ...
            public bool IgnoreFollowRotation = false;
        }
        ...
        private void OnPlayParticleSystem(PlayParticleSystem e)
        {
            ...
            if (parGO != null)
            {
                HNDParticleSystem fps = parGO.GetComponent<HNDParticleSystem>();
                if (fps != null)
                {
                    fps.Play(e.Config.Position, e.Config.NormalVector, e.Config.FollowTransform, e.Config.IgnoreFollowRotation, e.Config.Scale);
        ...
        IEnumerator ReturnObjectWithDelay(GameObject go, HNDParticleSystem ps, float delay)
        {
        ...
        IEnumerator ReturnObjectDelayed(GameObject go, HNDParticleSystem ps)
        {
        ...
        private HNDParticleSystem GetParticleSystemPrefabFromName(string name)
        {
            HNDParticleSystemData data = m_ParticleManagerData.ParticleSystems.Find(p => p.Name == name);
            if (data != null)
            {
