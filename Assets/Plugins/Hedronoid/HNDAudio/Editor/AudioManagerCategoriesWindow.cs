...
namespace Hedronoid.Audio
{
    public class AudioManagerCategoriesWindow : EditorWindow
    {
        ...
        private const bool ENABLE_LAMS = true;

        [MenuItem("Hedronoid/Audio Manager/Manager...")]
        ...
        void SaveLeafsToEnum(AudioManagerCategory root)
        {
            ...
            sw.WriteLine("\t\tstring soundId = null;" + System.Environment.NewLine + "\t\tswitch(s){");

            foreach (AudioManagerCategory cat in cats)
            {
                sw.WriteLine("\t\tcase Sounds." + cat.EnumString + ":");
                sw.WriteLine("\t\t\tsoundId = \"" + cat.ID + "\";");
                sw.WriteLine("\t\t\tbreak;");
            }

            sw.WriteLine("\t\t}" + System.Environment.NewLine + "\t\treturn soundId;");
            sw.WriteLine("\t}");
                    ...
                    EditorGUILayout.BeginVertical("box", options1);
                    {
                        ...
                        EditorGUILayout.BeginVertical("box", expand);
                        {
                            ...
                            EditorGUILayout.BeginHorizontal(GUI.skin.FindStyle("Toolbar"), expand);
                            {
                                ...
                                if (ENABLE_LAMS)
                                {
                                    if (GUILayout.Button("LAMS import", GUI.skin.FindStyle("toolbarbutton"), GUILayout.Width(120)))
                                    {
                                        var importerWindow = (AudioManagerImporterWindow)EditorWindow.GetWindow(typeof(AudioManagerImporterWindow), false, "LAMS import");
                                        importerWindow.Initialize(m_AudioManagerRootData);
                                    }
                                }
        ...
        private AudioManagerData LoadOrCreateAudioData()
        {
            ...
            if (data == null)
            {
                string dir = Path.GetDirectoryName(p.AudioManagerDataPath);
                ...
                PrefabUtility.SaveAsPrefabAsset(prefabGO, p.AudioManagerDataPath);
#else
